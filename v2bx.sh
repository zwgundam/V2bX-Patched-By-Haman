...
# 直链物理下载根路径 (已统一规划至 /v2bx/ 专属分类子目录)
BASE_URL="https://jp.671152.xyz/p/armjp%EF%BC%88local%EF%BC%89/root/.openclaw/workspace/haman-pub/v2bx"
...
update_v2bx() {
    ...
    # 2. 安全同步更新管理脚本本身 (先下载到临时文件，再 mv 原子覆盖，防止正在运行的脚本被截断导致语法错乱)
    echo -e "${CYAN}正在同步更新 v2bx 管理脚本本身...${RESET}"
    local tmp_script="/tmp/v2bx_update.sh"
    curl -fsSL -o "${tmp_script}" "${BASE_URL}/v2bx.sh" || wget -O "${tmp_script}" "${BASE_URL}/v2bx.sh"
